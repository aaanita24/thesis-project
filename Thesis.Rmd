...
# taller export for Occupation's labels
# ggsave("wage_plot.pdf", p, width = 8.5, height = 10.5, dpi = 300, bg = "white")
...
# ggsave("area_wage_plot.pdf", p2, width = 8, height = 4.8, dpi = 300, 
# bg = "white")
```
...
A "composition-adjusted" GPG will be calculated. For each NUTS-1 macro
area and each breakdown category (education level, contract type, hours
...
Visualizing the composition-adjusted gap (by macro areas):
...
# ggsave("agpg_map.pdf", p3, width = 8, height = 7.2, dpi = 300, bg = "white")
```
...
missing not at random scenario (MNAR). This incomplete data will be
imputed both using the standard MICE package, and using the LLMs. Only
the rows with the specified stratification type at the geographic level
will be kept (already specified with `ses_wide`).

-   low `reg_dis` level = \~0.10 probability of mean values being
    missing

-   medium = \~0.25 probability of mean values being missing

-   high = \~0.45 probability of mean values being missing
...
zero data context to define the prior knowledge that model carries about
Italian regional wages from training, to be used as a baseline. The MNAR
dataset will be used because the uninformed LLM ignores all observed
data, so the missingness mechanism doesn't change its guesses or
mechanism.
...
Free-form contextual prompt and structured table completion.

## 6.1 Contextual imputation

The model is given one observed row as context and asked to impute the
missing wage for the opposite sex. What is provided:

-   the counterpart-sex wage in the same area x category cell

<!-- -->

-   the national mean for that sex x category

-   the area-level mean for that sex from observed non-missing cells

-   all other observed areas' wages for the same sex x category
...
This prompt will be given only a small table with missing cells, testing
structured table completion and tries to understand whether structured
tabular framing (with an explicit MISSING cell) elicits better
imputation than bullet points.
...
## 7.1 Uninformed one-shot imputation

Uninformed baseline for the one-shot imputation method, any preserved accuracy or aGPG must come from the model's priors rather than the supplied data. Only a target table to fill will be provided to the LLM.
...
## 7.2 One-shot broad-context imputation

Phase 7 gives two layers of context at once, combining geographic
anchoring (from the observed MNAR cells) with structural/demographic
depth (from the reference). The imputation method is different, as the
LLM here is asked to do a one-shot imputation.

-   **Reference (context)**: `ses_data` rows with
    `strat_type != "geography"`. It provides a broader context to the
    LLM to predict from.
...
-   **Target (to impute)**: `ses_exp_mnar`,geography-stratified wages
    ...
    stratification axes, there is no overlap (already created for the baseline).
...
## 7.3 Anonymized geography imputation

To try and test leakage, the geographical areas will be masked into Area 1, 2 etc., to understand how the model performs when context is given but specific geographical indications are anonymized:

```{r anon-geo-imp}
# anon-geo: identical to 7.1, reuses ref_csv + sys_oneshot + mnar_long + missing_keys.
...
The unstructured baseline preserves the aggregate gender gap least faithfully (MAE 0.032, ρ 0.94); contextual reference improves it (MAE 0.023, ρ 0.97). Anonymising the macro-areas recovers roughly half the contextual error gain (MAE 0.026, RMSE 0.037), but its rank fidelity reverts to baseline (ρ 0.94). This could indicate that real area identity aided ordering more than level: partial, ordering-specific geographic memorization. Error concentrates in the most disadvantaged tier throughout: high-disadvantage MAE stays roughly an order of magnitude above the low tier (0.69/0.39/0.43 vs 0.10/0.05/0.04). Imputation degrades precisely where wage data is sparsest.

Comparison with MICE on MNAR performance:
Against the LLM one-shot conditions, MICE-PMM has a worst performance. Its aGPG MAE (0.107) is three to five times the LLM range (0.023-0.032), and its rank fidelity collapses to ρ 0.16 versus the LLMs' 0.94-0.97. The disadvantage gradient diverges most: MICE's high- and moderate-tier MAE reach 1.82 and 1.99, roughly three to four times the worst LLM tier (≈0.43-0.69) and over forty times its own low tier. PMM fails precisely where the LLMs degrade only modestly, which are the disadvantaged-areas.
