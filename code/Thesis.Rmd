...
The Structure of Earnings Survey for 2022 in Italy is a series of
aggregated tables. In order to understand how to navigate them, the
first sheet will be uploaded and looked at manually.
...
publish). The table will be pivoted and the names recoded to be clearer.
...
Table 1 is finalized: the labelled sector as an indicator, the
...
Now the breakdown variables that are used across the tables will be
labeled. SES classifies the tables according to meta-level variables:
education (like tav 1), contract type, occupation (ISCO), hours regime,
seniority brackets.
...
-   strat_type; the primary dimension the table stratifies workers on. 4
    ...
    -   sector: according to the European classification economic
        activity (NACE)
...
-   row_label; the specific category within strat_type.

-   sex; Female, Male, Total.

-   breakdown_var; the secondary dimension cross-tabulated against
    strat_type. 5 'meta' values:
...
`breakdown_cat`, and then kept as the `breakdown_cat` variable.

```{r ses-final-cleaning}
ses_data <- ses_data |>
...
RACLI data set has four main dimensions: geography (country / NUTS-1
geographical macro areas / 20 NUTS-2 regions / \~107 provinces), sex
(Male / Female / Total), year, and wage statistics (mean / p10 / median
/ p90).
...
local code, used in the RACLI data set, for reference.
...
-   The boxplot of `sex x outcome_var` shows that female earn less than
    males, for hourly and annual wages. The annual hours gap is smaller
    but still present, pointing to both a pay rate and a hours-worked
    component.

-   The correlation heatmap (hourly / annual wage / annual hours) shows
    hourly wages moderately correlated with annual wage, while hourly
    wage nearly uncorrelated with annual hours (possibly indicating that
    full-time hours are compressed regardless of pay).

-   Occupation shows the highest average wages and the widest
    Female-Male spread. Tenure and education both show clear wage
    gradients, while temporary contract and part-time show the lowest
    values.

```{r eda-data-viz, fig.width=8, fig.height=8,  fig.showtext = TRUE, dpi = 300}
# font 
...
Visualization per macro area:

```{r eda-data-viz, fig.width=7, fig.height=4}
nuts1_labels <- c(
...
A "within-category" GPG will be calculated. For each NUTS-1 macro area
and each breakdown category (education level, contract type, hours
regime, occupation, tenure bracket), compute the gender gap.
...
wGPG per NUTS-1 area, per breakdown variable. Weight within-category
gaps by the female-wage share of that category within the area, then sum
across categories.

```{r wGPG-area-breakdown}
...
A single wGPG number per area, taken as the mean across the five
breakdown variables.
...
Visualizing the within-category gap (by macro areas):

```{r fig.width=6, fig.height=6, fig.showtext = TRUE, dpi = 300}
...
total' cells at NUTS-2 regional level.

This index will be used to fabricate MNAR missingness among the data.
...
Initial baseline MICE imputation under the MCAR assumption, which
assumes that there is no relationship between missingness and either the
wage or the region, as each cell has the same probability of being
missing.
...
region, the information will be modified in order to fabricate a
missing-not-at-random (MNAR) scenario. Only the rows with the specified
stratification type at the geographic level will be kept (already
specified with `ses_wide`).

The drop probability assigned to each cell has two components:

-   **Tier component (MAR given area)**: each NUTS-1 area's
    `reg_dis_lvl` sets a baseline drop rate; \~0.10 for low-disadvantage
    areas, \~0.25 for moderate, and \~0.45 for high. Because
    `reg_dis_lvl` is a deterministic function of `nuts1`, and `nuts1` is
    always observed and supplied to MICE as a predictor, this component
    alone would only be MAR conditional on area.

-   **Value component (`beta`, actual MNAR)**: within each tier, cells
    whose true wage sits below that tier's median get an additional
    `beta * (median - value)` to their drop probability. The probability
    of a cell being missing depends on the value being hidden itself,
    even after conditioning on area/tier.

Combined MAR-MNAR mechanism.
...
The PMM approach is applied to the MNAR-engineered SES data set.
...
-   **Cell-level SES table**: true vs imputed wage and bias, sorted by
    largest error as well.
...
-   **Fairness by disadvantage tier:** the mean bias and MAE broken down
    by low / moderate / high disadvantage regions.

# PHASE 5

## 5.1 LLM API call

The selected LLM will be `Qwen 3-32B`. An API key was obtained for 5
dollars (of tokens) from OpenRouter.com.
...
## Shared one-shot imputation runner

To ensure comparison, both LLM conditions below (uninformed and
anonymized-geography) are run through a single function,
`oneshot_run()`. It is given only a target table `d` and no external
reference table, no extra curated anchors and no national "Total"
aggregates computed from `ses_wide`. The only difference between the two
conditions is whether the NUTS-1 area names shown to the model are real
or replaced with `Area_1-Area_5`.
...
## 6.1 Uninformed one-shot imputation

Uninformed baseline for the one-shot imputation method: any preserved
accuracy or wGPG must come from the model's priors rather than the
supplied data.

```{r oneshot-uninformed}
set.seed(240603)
...
## 6.2 Anonymized geography imputation

This condition tests whether the uninformed result in 6.1 reflects
learned wage structure or memorized ISTAT geography. The NUTS-1 area
names in the target table are replaced with `Area_1-Area_5`.

`oneshot_run()` keeps track of the real area identities via
`missing_keys`, so the results below map back onto the true NUTS-1 areas
for evaluation even though the model never saw their real names.
...
The comparison of this thesis: uninformed LLM vs. anonymized-geography
LLM vs. MICE, on the MNAR data.
...
Of the two LLM conditions, the **anonymized-geography** run is the more
informative one: masking the NUTS-1 names is what separates *wage
structure the model has learned* from *ISTAT geography the model has
memorized*. If the **uninformed** condition alone reaches a high
Spearman correlation on the regional ranking, that is not by itself
evidence the model "recovered" the missing structure -- it may simply
have recognized "Nord-Ovest" / "Sud" etc. and recalled Italy's
North-South wage gradient from training data. The anonymized condition
removes that shortcut, which is why -6.1 vs. -6.2 (not the absolute
level of either) is the result that supports a claim about imputation
ability rather than memorization.

# PHASE 7

Steps to check whether LLM's performance draws on training data, and to
therefore give credibility to the findings.

## 7.1 Iteration

The MNAR mechanism is re-drawn across many seeds and re-runs MICE (50
redraws) and the LLM (more costly, 8 redraws) on each one, reporting the
mean and SD of the wGPG MAE and Spearman correlation vs. `ses_wide` for
every method. Iteration here is employed to investigate the methods'
stability in reproduction and imputation, and their deviation.
...
Because the data contains published 2022 ISTAT figures the LLM may have
seen during training, so "estimation from the observed cells" and
"retrieval of memorized values" are entangled. To separate them, every
value in `ses_wide` is perturbed by a fixed multiplicative shift
(`Female * 1.12`, `Male * 1.18`) before re-running the MNAR mechanism
and the uninformed LLM imputation.

If the LLM's imputed values land closer to what the masked table
actually implies than to the original published truth (what the model
may have memorized) –\> genuine **estimation** from the given data. If
the reverse holds, so imputations land closer to the original 2022
figures than to the perturbed table actually shown to the model, that
points to **retrieval/memorization** rather than estimation.
...
`score()` measures how well the imputed gender pay gaps match the true
ones. Here, the perturbed data \> original data -\> estimation rather
than retrieval: it indicates that the model is computing the gaps from
the observed cells it was given.

```{r tables-iteration-unseen-test}
...
## 7.3 Covariate deletion

Strip covariate information to see if error rises. The breakdown columns
from the MICE run are also stripped and degradation of each is compared,
as it is assumed that both methods have here access to the same data
(lacking the breakdown categories). Additional imputation to increase
credibility.
...
# write_csv(llm_blind_merged, "llm_blind_merged.csv")
```

Blind MICE to ensure comparison:
...
MAE comparison (does the imputation stripped of covariate's MAE stay
similar to the one with covariates for the LLM? -\> the model is then
drawing from memory rather than evaluating):
...
With covariates the LLM imputes almost exactly (MAE 0.86 EUR/hour) and
far better than MICE (7.96), but once they are stripped its accuracy
collapses to 5.40 --\> performance depends on the data it is shown
rather than on memorized values.
