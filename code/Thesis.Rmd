...
Phase 0 deals with the data cleaning and preparation of the real data of
the thesis from two ISTAT sources: SES income data and RACLI regional
data from 2022. The output is three tidy datasets: *'ses_data',
'racli_clean'*, and *'regional_codes',* that together describe the
structure of Italian wages along the demographic, sectoral, and
geographic macro and micro areas.
...
All tables are mapped to their specific attributes that are needed in
the functions. These information are taken from each table, that specify
the type of outcome and breakdown variable they refer to.
...
-   The box plot of `sex x outcome_var` shows that female earn less than
...
-   The correlation heat map (hourly / annual wage / annual hours) shows
    hourly wages moderately correlated with annual wage, while hourly
    wage nearly uncorrelated with annual hours.
...
```{r eda-viz, fig.width=7, fig.height=4}
nuts1_labels <- c(
...
wage or the region, each cell has the same probability of being missing.
...
PMM is applied to the MNAR-engineered SES data set.
...
-   The **mean bias and MAE broken down** by low / moderate / high
...
reference table, no national "Total" aggregates computed from
`ses_wide`. The only difference between the two conditions is whether
the NUTS-1 area names are shown to the model.
...
accuracy or wGPG comes from the model's priors rather than the supplied
data.
...
give credibility to the findings.

## 7.1 Iteration

The MNAR mechanism is re-drawn across many seeds and re-runs MICE (50
redraws) and the LLM (more costly, 8 redraws), reporting the mean and SD
of the wGPG MAE and Spearman correlation vs. `ses_wide` for every
method. Employed to investigate the methods' robustness, and their
deviation.
...
value in `ses_wide` is altered by multiplying (`Female * 1.12`,
`Male * 1.18`) before re-running the MNAR mechanism.

If the LLM's imputed values are closer to what the masked table actually
implies than to the original truth - memorized –\> genuine
**estimation** from the given data. Otherwise,
**retrieval/memorization** rather than estimation.
...
from the MICE run are also stripped and MAE of each is compared.
...
pred["Female", "nuts1"] <- 1 # Female predicted from area
pred["Male",   "nuts1"] <- 1 # Male predicted from area
...
MAE comparison (does the stripped imputation's MAE stay similar to the
complete one for the LLM? -\> the model is then drawing from memory
...
Faceted area map reproducing wGPG reconstruction for each imputation method, compared to the original. 
```{r results-viz fig.width=6, fig.height=6, fig.showtext = TRUE, dpi = 300}
# recreation of wGPG across methods vs original
# per-area wGPG
area_wgpg <- function(df) {
  df |>
    group_by(area, breakdown_var) |>
    mutate(gap    = 1 - Female / Male,
           weight = Female / sum(Female, na.rm = TRUE)) |>
    summarise(wGPG = sum(weight * gap, na.rm = TRUE), .groups = "drop") |>
    group_by(area) |>
    summarise(wGPG = mean(wGPG), .groups = "drop")
}

method_levels <- c("Original", "MICE", "LLM one-shot", "LLM anonymised")

wGPG_methods <- bind_rows(
  wGPG_area                              |> mutate(method = "Original"),
  area_wgpg(ses_mice_mnar)               |> mutate(method = "MICE"),
  area_wgpg(llm_baseline_oneshot_merged) |> mutate(method = "LLM one-shot"),
  area_wgpg(llm_anon_merged)             |> mutate(method = "LLM anonymised")) |>
  mutate(method = factor(method, levels = method_levels)) |>
  left_join(nuts1_codes, by = c("area" = "nuts1"))

# join the 4-method long table onto one geometry (5 areas -> 20 rows, sf retained)
map_levels <- italy_nuts1 |>
  select(NUTS_ID) |>
  left_join(wGPG_methods, by = "NUTS_ID", relationship = "one-to-many")

p4 <- ggplot(map_levels, aes(fill = wGPG)) +
  geom_sf(colour = "white", linewidth = 0.2) +
  facet_wrap(~ method, nrow = 2) +
  scale_fill_gradient2(
    low = "#542788", mid = "#f7f7f7", high = "#b2182b",
    midpoint = 0, limits = c(-0.12, 0.12),
    oob = scales::squish,                      
    labels = scales::percent_format(accuracy = 1),
    name = "wGPG\n(% of male pay)") +
  labs(title    = "Within-category gender wage gap: original vs imputed",
       subtitle = "NUTS-1 macro-areas of SES 2022; shared colour scale across 
       panels") +
  theme_void(base_size = 11, base_family = "ebgaramond") +
  theme(plot.title.position = "plot",
        ...
        strip.text    = element_text(face = "bold", size = 11, 
                                     margin =  margin(b = 3)),
        plot.margin   = margin(t = 14, r = 6, b = 6, l = 6),
        legend.title  = element_text(size = 10),
        legend.text   = element_text(size = 9),
        legend.key.height = unit(0.9, "lines"),
        legend.key.width  = unit(0.7, "lines"))
